...
| `npm run fetch:nagoya` | 名古屋市会（抜粋＋原本リンク） |
...
- **名古屋市会**：robots は `/tenant/` 許可だが、全文の外部公開は市へ要許諾 →
  本サイトは**抜粋＋AI要約＋原本リンク**に留める。
...
components/   UIコンポーネント          lib/sources/ ソース・アダプタ（kokkai/discussvision/linkout/…）
